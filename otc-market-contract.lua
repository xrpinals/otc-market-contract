...
offline function M:sell_orders(param_str: string)
    ...
    let from_idx: int = tointeger(params[1])
    let page_count: int = tointeger(params[2])
    var end_idx: int = from_idx + page_count
    let orders_indexes: Array<string> = self.storage.orders_indexes

    if end_idx >= #orders_indexes then
        end_idx = #orders_indexes
    end

    let orders: Array<string> = []
    for i = from_idx, end_idx do
        let order_idx: string = tostring(orders_indexes[i])
...
function M:on_deposit_asset(json_str: string)
    ...
    let sell_amount: int = tointeger(json_arg.num)
    let sell_symbol: string = tostring(json_arg.symbol)
    -- param: SELL,WantToBuyAssetSymbol,WantToBuyAssetAmount
    -- param: BUY,OrderIdx
    let param_str = tostring(json_arg.param)

    let storage_base_symbol: string = self.storage.base_symbol
    let storage_quote_symbol: string = self.storage.quote_symbol
    let storage_top_order_index: int = self.storage.top_order_index
    let storage_owner: string = self.storage.owner
    let storage_orders_indexes: Array<string> = self.storage.orders_indexes
    ...
    if (sell_symbol ~= storage_base_symbol) and (sell_symbol ~= storage_quote_symbol) then
        return error("Params Error: invalid sell_symbol")
    end
    ...
    if (#params == 3) then
        ...
        let buy_symbol: string = tostring(params[2])
        let buy_amount: int = tointeger(params[3])

        if (buy_symbol ~= storage_base_symbol) and (buy_symbol ~= storage_quote_symbol) then
        ...
        sell_order["order_index"] = tostring(storage_top_order_index)
        let r = json.dumps(sell_order)
        
        let event_str = caller_address .. "," .. tostring(storage_top_order_index) .. "," .. tostring(sell_symbol) .. "," .. tostring(sell_amount) .. "," ..  tostring(buy_symbol) .. "," .. tostring(buy_amount)
        emit PlaceOrder(event_str)

        fast_map_set("sell_orders", tostring(storage_top_order_index), tostring(r))
        self.storage.orders_indexes[#storage_orders_indexes + 1] = tostring(storage_top_order_index)
        self.storage.top_order_index = storage_top_order_index + 1
    else
        if (params[1] ~= "BUY") then
            return error("Params Error: invalid params")
        end

        let order_idx: string = tostring(params[2])
        var found: bool = false
        for k, v in pairs(storage_orders_indexes) do
            if v == order_idx then
                found = true
                break
            end
        ...
        if (sell_symbol == sell_order["buy_symbol"]) and (tostring(sell_amount) == sell_order["buy_amount"]) then
            ...
            -- transfer to seller
            let res1: int = transfer_from_contract_to_address(tostring(sell_order["seller"]), sell_symbol, need_transfer_amount)
            if res1 ~= 0 then
                return error("Transfer asset " .. sell_symbol .. " to " .. tostring(sell_order["seller"]) .. " error, code: " .. tostring(res1))
            end

            let event_str1: string = tostring(sell_order["seller"]) .. "," ..  tostring(sell_order["sell_symbol"]) .. "," .. tostring(sell_order["sell_amount"]) .. "," ..  tostring(sell_symbol) .. "," .. tostring(sell_amount)
            emit Exchange(event_str1)

            if need_transfer_fee > 0 then
                -- transfer to owner
                let res_fee: int = transfer_from_contract_to_address(storage_owner, sell_symbol, need_transfer_fee)
                if res_fee ~= 0 then
                    return error("Transfer fee " .. sell_symbol .. " to " .. storage_owner .. " error, code: " .. tostring(res_fee))
                end

                let event_fee: string = tostring(sell_order["seller"]) .. "," .. tostring(sell_symbol) .. "," .. tostring(need_transfer_fee)
                emit ExchangeFee(event_fee)
            end

            -- transfer to buyer
            let res2: int = transfer_from_contract_to_address(caller_address, tostring(sell_order["sell_symbol"]), tointeger(sell_order["sell_amount"]))
            if res2 ~= 0 then
                return error("Transfer asset " .. tostring(sell_order["sell_symbol"]) .. " to " .. caller_address .. " error, code: " .. tostring(res2))
            end

            let event_str2: string = caller_address .. "," ..  tostring(sell_symbol) .. "," .. tostring(sell_amount) .. "," ..  tostring(sell_order["sell_symbol"]) .. "," .. tostring(sell_order["sell_amount"])
...
function M:cancel_order(order_idx: string)
    ...
    -- transfer to seller
    let res: int = transfer_from_contract_to_address(tostring(sell_order["seller"]), tostring(sell_order["sell_symbol"]), tointeger(sell_order["sell_amount"]))
    if res ~= 0 then
        return error("Transfer asset " .. tostring(sell_order["sell_symbol"]) .. " to " .. tostring(sell_order["seller"]) .. " error, code: " .. tostring(res))
    end

    let event_str: string = tostring(sell_order["seller"]) .. "," ..  tostring(sell_order["sell_symbol"]) .. "," .. tostring(sell_order["sell_amount"])
...
function M:set_fee_rate(fee_rate_str: string)
    ...
    let fee_rate: int = tointeger(fee_rate_str)
    ...
    let event: string = tostring(fee_rate)
...
function M:open_market(param_str: string)
    ...
    let base_symbol: string = tostring(params[1])
    let quote_symbol: string = tostring(params[2])
    ...
    if (storage_base_symbol ~= "") or (storage_quote_symbol ~= "") then
    ...
    let event: string = storage_base_symbol .. "-" .. storage_quote_symbol
...
function M:close_market()
    ...
    let event: string = self.storage.base_symbol .. "-" .. self.storage.quote_symbol
